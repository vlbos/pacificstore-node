...
async function main() {
    ...
    try {
        ...
        const executeOrderbook = api.registry.createType("Role", { pallet: 'Orderbook', permission: 'Execute' });
        submit(api, api.tx.rbac.createRole(`Orderbook`, 'Execute'), users.admin);


        const executeWyvernExchange = api.registry.createType("Role", { pallet: 'WyvernExchange', permission: 'Execute' });
        submit(api, api.tx.rbac.createRole(`WyvernExchange`, 'Execute'), users.admin);

        const executeSumStorage = api.registry.createType("Role", { pallet: 'SumStorage', permission: 'Execute' });
        submit(api, api.tx.rbac.createRole(`SumStorage`, 'Execute'), users.admin);
        ...
        console.log("======1====67=====");
        submit(api, api.tx.rbac.assignRole(users.bob.key.address, executeOrderbook), users.admin);
        submit(api, api.tx.rbac.assignRole(users.betty.key.address, executeOrderbook), users.admin);
        submit(api, api.tx.rbac.assignRole(users.charlie.key.address, executeOrderbook), users.admin);
        submit(api, api.tx.rbac.assignRole(users.dave.key.address, executeOrderbook), users.admin);
        submit(api, api.tx.rbac.assignRole(users.eve.key.address, executeOrderbook), users.admin);
        submit(api, api.tx.rbac.assignRole(users.ferdie.key.address, executeOrderbook), users.admin);


        submit(api, api.tx.rbac.assignRole(users.bob.key.address, executeWyvernExchange), users.admin);
        submit(api, api.tx.rbac.assignRole(users.betty.key.address, executeWyvernExchange), users.admin);
        submit(api, api.tx.rbac.assignRole(users.charlie.key.address, executeWyvernExchange), users.admin);
        submit(api, api.tx.rbac.assignRole(users.dave.key.address, executeWyvernExchange), users.admin);
        submit(api, api.tx.rbac.assignRole(users.eve.key.address, executeWyvernExchange), users.admin);
        submit(api, api.tx.rbac.assignRole(users.ferdie.key.address, executeWyvernExchange), users.admin);



        submit(api, api.tx.rbac.assignRole(users.bob.key.address, executeSumStorage), users.admin);
        submit(api, api.tx.rbac.assignRole(users.betty.key.address, executeSumStorage), users.admin);
        submit(api, api.tx.rbac.assignRole(users.charlie.key.address, executeSumStorage), users.admin);
        submit(api, api.tx.rbac.assignRole(users.dave.key.address, executeSumStorage), users.admin);
        submit(api, api.tx.rbac.assignRole(users.eve.key.address, executeSumStorage), users.admin);
        submit(api, api.tx.rbac.assignRole(users.ferdie.key.address, executeSumStorage), users.admin);
        ...
        const beef1 = uuidv4();
        submit(api, api.tx.orderbook.postOrder(beef1, users.bob.key.address, [['desc', 'beef burger']]), users.betty);
        const beef1 = uuidv4();
        submit(api, api.tx.orderbook.postAssetWhiteList('users.bob.key.address', 'beef burger',"test@test.com"), users.betty);
