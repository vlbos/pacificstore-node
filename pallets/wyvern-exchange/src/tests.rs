...
fn make_order_ex(
	...
	fee_recipient: AccountId,
	side: u8,
) -> (Vec<AccountId>, Vec<u64>, FeeMethod, Side, SaleKind, HowToCall, Vec<u8>, Vec<u8>, Vec<u8>) {
...
fn require_valid_order_ex() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(
			hash,
			vec![
				144, 223, 135, 224, 228, 211, 197, 17, 23, 67, 219, 239, 162, 220, 227, 194, 8, 77,
				64, 59, 252, 167, 84, 189, 45, 125, 52, 218, 160, 131, 2, 14
			]
		);
	});
}
...
fn cancel_order_ex_with_signature() {
	new_test_ext().execute_with(|| {
		...
		assert!(System::events()
			.iter()
			.any(|er| er.event == TestEvent::exchange_core(RawEvent::OrderCancelled(hash.clone()))));
	});
}
...
fn transfer_tokens() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(result);
		assert_eq!(
			<Test as wyvern_exchange::exchange_common::Config>::Currency::free_balance(&sender),
			99999999999999958
		);
		assert_eq!(
			<Test as wyvern_exchange::exchange_common::Config>::Currency::free_balance(&sender1),
			100000000000000042
		);
	});
}
