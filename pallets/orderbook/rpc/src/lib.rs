...
pub fn convert_json_to_orderquery<AccountId>(
    ...
    if let Some(json) = order_query_json {
        ...
        if let Some(params) = json.params {
            order_query.params = Some(
                params
                    .into_iter()
                    .map(|t| OrderField::new(&from_hex(t.name), &from_hex(t.value)))
                    .collect(),
            );
        }
    ...
    Moment: Codec,
{
    ...
    fn get_orders(
        ...
        let runtime_api_result = api.get_orders(
            &at,
            convert_json_to_orderquery::<AccountId>(order_query),
            page,
        );
        runtime_api_result.map_err(|e| RpcError {
            code: ErrorCode::ServerError(9876), // No real reason for this value
    ...
    fn get_asset(
        ...
        let runtime_api_result = api.get_asset(
            &at,
    ...
    fn get_assets(
        ...
        let runtime_api_result = api.get_assets(
            &at,
            convert_json_to_assetquery::<AccountId>(asset_query),
            page,
        );
        runtime_api_result.map_err(|e| RpcError {
            code: ErrorCode::ServerError(9876), // No real reason for this value
