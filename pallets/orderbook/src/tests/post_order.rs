...
fn set_order_limits() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(result);
        let limits = 1000;
        let result = Orderbook::set_order_limits(Origin::signed(sender), limits);

        assert_ok!(result);

        assert_eq!(<OrderLimits>::get(), limits,);

        // Event is raised
        assert!(System::events()
            .iter()
            .any(|er| er.event == TestEvent::orderbook(RawEvent::OrderLimitsChanged(limits))));
    });
}
...
fn set_asset_white_list_limits() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(result);
        let limits = 2000;
        let result = Orderbook::set_asset_white_list_limits(Origin::signed(sender), limits);

        assert_ok!(result);

        assert_eq!(<AssetWhiteListLimits>::get(), limits,);
...
fn remove_order_on_chain() {
    new_test_ext().execute_with(|| {
        ...
        let result = Orderbook::remove_order(Origin::signed(sender), index);
