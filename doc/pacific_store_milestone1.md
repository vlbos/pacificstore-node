...
- `pub fn require_valid_order(order: &OrderType<T::AccountId, T::Moment, BalanceOf<T>>,sig: &T::Signature,) -> Result<Vec<u8>, Error<T>>`: Assert an order is valid and return its hash.
